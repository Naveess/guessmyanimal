...
The fixed nav floats above every page rather than occupying flow space, so nothing under it clears it automatically. `--nav-h` + `--page-top` are the two tokens every page's content container clears against (`calc(var(--nav-h) + var(--page-top))`), rather than each page guessing its own top padding. Both step up together at the 860px breakpoint, when the nav grows from a corner button into a taller centred pill. The splash is the one deliberate exception — it floats the nav over the Sun ground on purpose and never pays this padding.
...
### Stacking
One shared z-index scale, low to high, instead of ad-hoc numbers per component:

| Token | Value | Band |
|---|---|---|
| `--z-sticky` | 20 | In-feed sticky surfaces (the party mini-strip) |
| `--z-topbar` | 30 | The sticky, frosted top bar |
| `--z-dropdown` | 40 | A dropdown floating over the top bar (search suggestions) |
| `--z-scrim` | 45 | The dimming layer behind an open panel |
| `--z-panel` | 50 | A floating panel itself (share panel) |
| `--z-chrome` | 60 | The fixed corner/nav controls — always wins |

A component's own internal paint order (e.g. a badge sitting over its own icon) stays a plain small literal; these tokens are only for chrome that can collide with *other* chrome across the page.
...
- **Structure:** `.wip-head` (a mark, a name, and a `.wip-tag` pushed right on `margin-left: auto`), a `.wip-note` in plain language, then the live control it warned about. Border, no shadow — it sat flush in the setup flow.
- **Never Sun.** Warn is the palette's *other* yellow-ish register, far enough from Sun in saturation that it never reads as brand — but a solid-Sun element in the same panel would have broken the One-Yellow Rule and made a warning look like a call to action.
- **The control stayed live.** The panel was a sign, not a wall: a half-built path still has to be testable. Disabling the input would have been a separate, louder decision.
- **Third-party marks** were drawn with `currentColor` like every other icon on the site. That rule outlived the panel — the Twitch glyph on the new Connect button follows it too.
...
- **The landing/pitch screen** (pre-connect state): a hero headline and one-line pitch, three reassurance points and a 3-step how-it-works as plain Card+Line tiles (the Border-Or-Shadow Rule, not a new frame), and a You/Chat role breakdown — all built from primitives that already exist elsewhere on this same page, most visibly the numbered/tick badge, which is `.party-hint-n`'s own circle reused rather than a new icon language, so a first-time visitor and a mid-round streamer read one consistent system.
...
- **Chat status** (`.streamer-chatstatus`): three states on the existing semantic colours — Good once the join is confirmed, Warn while still waiting to hear back, Bad for a refusal or a dropped connection. The middle state earns its place: the old version went green the instant `JOIN` was sent, so a typo'd channel looked connected forever. "Waiting" must never be drawable as "working".
- **Chat leaderboard** (`.streamer-board`): deliberately *not* `.party-score`. Top few only, no presence dot (nothing to heartbeat from), and a fixed chat glyph in place of a picked icon for anyone who guessed from chat. Rows scale with the viewport (`clamp(17px, 2.6vw, 22px)`) for the same reason the Mystery Animal presentation scale and the party join code do: it's read off someone else's screen, often a compressed one.
- **"Which one is me"** (`.streamer-board-row.is-mine`): the streamer's own row, once they've guessed — the same read-only Sun tint `.party-score.is-mine` spends on local-play identity, spent here on the one identity this surface actually has.
- **Round winner** (`.streamer-board-row.is-winner`): the same tint again, spent on the moment instead of on identity — "who just won" is what a chat is watching for. Crown plus a Sun-deep points recolour, never a fill, so the One-Yellow Rule holds with the Connect button. Can land on the same row as `.is-mine` (the streamer winning their own round) without conflict — both read as the same signal, not two competing ones.
- **Everything else is borrowed, not reinvented**: the Photo Card, the hint list, the guess form, and the result card and its win burst are the same components the other modes use. A round should look like a round wherever it's played; only the parts that answer a genuinely different question are new.
...
- **Desktop nav bar (≥860px):** the corner button and its dropdown become a single always-visible bar — no reason to hide the options behind a click when there's width to just show them. Centred at the top of the page rather than pinned to a corner, and drawn as a glass pill (translucent Card tint + `blur(18px)`, no border — the Border-Or-Shadow Rule's `--shadow-l` alone defines the edge) rather than a solid one, the same frosted language the sticky top bar already spends over the feed. Four of the five labels (About, Browse, Party, Stream) shorten for the row; Mystery Animal has no shorter form to fall back to. Mobile's dropdown has the vertical room to spell every one out in full and never sees the short form. Sound is its own fixed icon button next to the corner menu, not a dropdown item — see the Sound Toggle entry below.
- **Gloss (`--gloss-inset`):** the pill's `box-shadow` carries a second, inset layer on top of `--shadow-l` — a whisper of light along its top inside edge, themed separately per theme rather than derived by opacity (a plain wash nearly vanishes once `--card` goes near-black in dark mode). This is a property of the glass *material itself*, the same way the `blur(18px)` is, not a second edge treatment stacked on the shadow — so it doesn't reopen the Border-Or-Shadow Rule, and it isn't the glow the Do's and Don'ts rule out either: a specular highlight sits inside the shape's own edge instead of radiating outward from it.
- **Current page:** whichever item points at the page you're already on carries `aria-current="page"`, styled with the same Flat-bg-plus-weight treatment `.party-score.is-mine` uses for "this one, not the others" — read-only state, not a fill, so it never competes with the pill's own One-Yellow budget. Announced for free to a screen reader through the same attribute that drives the style.

### Party Mode surface
Party Mode is the one screen designed to be looked at by several people on several devices at once, and its components exist to answer questions a solo screen never raises: who else is here, what have they already tried, which one am I, who's ahead.
- **Panel framing** (`.party-scores`, `.party-feed`): each gets its own Card+Line frame (the Border-Or-Shadow Rule), not a flush list bleeding into the page — these are the two things a shared room actually watches, so they read as panels, not leftover rows.
- **Leaderboard rows** (`.party-score`): a rank, an emoji, a name, and tabular-nums points. Rows inside the panel are separated by a 1px Line top-border rather than boxed individually — a roster is one object, not a stack of cards.
...
- **Leader / round-winner** (`.party-score.is-leader`, `.is-winner`): a small crown glyph plus a Sun-deep recolour of the points figure — a text/icon recolour, not a fill, so it doesn't compete with the One-Yellow Rule the way a second solid-Sun element would. `is-leader` is persistent (top of the board by score); `is-winner` is a transient flag on whoever took the *last* round. They're allowed to overlap on the same row without conflict — both are "this player, positively" signals, not opposing states.
- **Presence dot** (`.party-score-dot`, `.is-online`): a Good-coloured dot with a soft expanding-ring pulse (`@keyframes party-pulse`), so "here right now" reads as alive rather than a static coloured bullet. Respects `prefers-reduced-motion` (the ring simply doesn't animate). Absence is never drawn as an error — a closed tab just stops refreshing and the dot goes out.
- **Twitch names** used to carry a `.is-twitch` marker on this board, back when chat guesses and phone guesses shared one room. A session is now one or the other, so the marker is gone — a board where every row is the same kind of player doesn't need a column saying so.
- **Activity feed** (`.party-feed`): every guess, right or wrong, as a chat-style log. New rows animate in (`rise`, the same entrance the hint list uses — one motion language, not two). A correct guess gets more than a colour swap: a one-shot brightness flash down to its resting Good-bg, plus a weight step on the text, since it's the feed's actual payoff moment and had been reading identically to a routine wrong guess.
- **Round divider** (`.party-feed-divider`): recoloured Sun-deep rather than Muted, the same "state via recolour" move the crown makes — a fresh round gets a small beat instead of reading as a plain label.
- **Mobile mini-strip** (`.party-minibar`): sticky, appears only once `#partyRoundhead` has scrolled out from under the top bar (an `IntersectionObserver`, re-armed on every `enterPage()` since the top bar's real height moves with `env(safe-area-inset-top)` and can't be hardcoded). Shows your rank, score, and how many are online, so scrolling down to read the leaderboard doesn't mean losing your own place in the round. Suppressed entirely at the desktop breakpoint, where the sticky side column already shows the same information permanently.
